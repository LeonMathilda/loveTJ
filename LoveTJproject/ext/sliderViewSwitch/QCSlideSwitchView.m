...
- (void)initValues
{
    ...
    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, _topScrollView.frame.size.height - 1 ,self.frame.size.width, 0.5)];
    lineView.backgroundColor = [UIColor colorWithRed:28.0/255.0 green:27.0/255.0 blue:32.0/255.0 alpha:1.0];
...
- (void)createNameButtons
{
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        //设置按钮尺寸
        [button setFrame:CGRectMake(xOffset,6,
                                    textSize.width, 20)];
        //计算下一个tab的x偏移量
        ...
        [button setTag:i+100];
        if (i == 0) {
            _shadowImageView.frame = CGRectMake(kWidthOfButtonMargin-1, 29, textSize.width, _shadowImage.size.height);
            button.selected = YES;
        }
...
- (void)selectNameButton:(UIButton *)sender
{
    ...
    if (!sender.selected) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            [_shadowImageView setFrame:CGRectMake(sender.frame.origin.x -1 , 29, sender.frame.size.width, _shadowImage.size.height)];
...
- (void)adjustScrollViewContentX:(UIButton *)sender
{
    ...
    if (sender.frame.origin.x - _topScrollView.contentOffset.x > self.bounds.size.width - (kWidthOfButtonMargin+sender.bounds.size.width)) {
        //向左滚动视图，显示完整tab文字
        [_topScrollView setContentOffset:CGPointMake(sender.frame.origin.x - (_topScrollView.bounds.size.width- (kWidthOfButtonMargin+sender.bounds.size.width)), -7)  animated:YES];
    }
    ...
    if (sender.frame.origin.x - _topScrollView.contentOffset.x < kWidthOfButtonMargin) {
        ...
        [_topScrollView setContentOffset:CGPointMake(sender.frame.origin.x - kWidthOfButtonMargin, -7)  animated:YES];
    }
}
